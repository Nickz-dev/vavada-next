          ...
          <script type="application/ld+json">
            {JSON.stringify({
              ...
              "itemListElement": [
                ...
                {"@type": "ListItem", "position": 3, "name": "vavada-casino.site"}
              ]
            })}
          </script>
        </Head>
        ...
        <main>
          ...
          <div className="container mx-auto px-4 py-8">
            <article className="prose prose-invert max-w-none">
              <h1 id="top" className="text-4xl md:text-5xl font-bold text-white mb-8 text-center">
                {lang === "ru" 
              ...
              <nav className="bg-[#2a2a42] rounded-xl p-6 mb-8">
                ...
                <ul className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {[
                    { id: "mirrors", icon: "🔗", title: translations.mirrors?.nav?.mirrors || "Актуальные зеркала", desc: translations.mirrors?.nav?.mirrorsDesc || "Рабочие ссылки на сегодня" },
                    { id: "how-to-use", icon: "📱", title: translations.mirrors?.nav?.howTo || "Как использовать", desc: translations.mirrors?.nav?.howToDesc || "Инструкция по входу" },
                    { id: "benefits", icon: "⭐", title: translations.mirrors?.nav?.benefits || "Преимущества", desc: translations.mirrors?.nav?.benefitsDesc || "Почему стоит использовать" },
                    { id: "faq", icon: "❓", title: translations.mirrors?.nav?.faq || "FAQ", desc: translations.mirrors?.nav?.faqDesc || "Ответы на вопросы" }
                  ].map((item) => (
                    <li key={item.id}>
                      <a href={`#${item.id}`} className="block p-4 bg-[#1c1c2d] rounded-lg hover:bg-[#282842] transition-colors">
                        <span className="text-[#ff424d] text-lg font-medium">{item.icon} {item.title}</span>
